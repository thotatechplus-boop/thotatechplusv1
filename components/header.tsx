    ...
    <header className="border-b border-neutral-800 bg-black/40 sticky top-0 backdrop-blur z-50">
      <div className="container flex items-center justify-between">
        <Link href="/home" className="flex items-center gap-3">
          <Image src="/logo.png" alt="THOTATECH+" width={200} height={200} />

        </Link>
        <nav className="flex items-center gap-4 text-sm">
          {nav.map((n) => (
            <Link key={n.href} href={n.href} className={`px-3 py-2 rounded-xl hover:bg-neutral-800 ${pathname === n.href ? "bg-neutral-800" : ""}`}>
              {n.label}
            </Link>
          ))}
